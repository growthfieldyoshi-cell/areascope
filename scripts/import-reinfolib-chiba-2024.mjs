/**
 * 不動産情報ライブラリ API (XIT001) — 千葉県 通常市36市 / 2024年 全四半期 の取得・集計スクリプト。
 *
 * 対象: 千葉県の通常市36市 × 2024 Q1〜Q4。千葉市(政令市)・町村は対象外。
 * 処理: 市×四半期ごとに API取得 -> raw バッチ置換 -> quarterly集計 UPSERT。
 *       全市完了後に annual (2024年通年) 集計を UPSERT。
 *
 * 実行: node --env-file=.env.local scripts/import-reinfolib-chiba-2024.mjs
 *
 * 注意:
...
// 千葉県 通常市36市の5桁JISコード (千葉市12100は対象外)。annual集計はこの全36市が対象。
const ALL_JIS = [
  "12202", "12203", "12204", "12205", "12206", "12207", "12208", "12210",
  "12211", "12212", "12213", "12215", "12216", "12217", "12218", "12219",
  "12220", "12221", "12222", "12223", "12224", "12225", "12226", "12227",
  "12228", "12229", "12230", "12231", "12232", "12233", "12234", "12235",
  "12236", "12237", "12238", "12239",
];
// 今回 raw取得・quarterly集計する対象 (前回 接続タイムアウトで中断した残り19市)。
// 完了済み17市 (12202〜12220) はバッチ置換で冪等のため、再投入が必要なら ALL_JIS に変更可。
const TARGET_JIS = [
...
try {
  ...
  console.log(`千葉県 通常市 ${cities.length}市 (今回処理分) × ${QUARTERS.length}四半期 を処理`);
  ...
  console.log("=== raw: property_type別 (千葉36市/2024) ===");
  ...
  console.log("=== raw: price_category別 (千葉36市/2024) ===");
  console.table((await client.query(
  ...
  console.log("=== annual: is_low_sample 件数 (千葉36市) ===");
  console.table((await client.query(
  ...
  console.log("=== 表示対象: annual transaction で表示3種を持つ千葉36市の数 ===");
