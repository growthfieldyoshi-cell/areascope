/**
 * 不動産情報ライブラリ API (XIT001) — 東京都 23区 / 2024年 全四半期 の取得・集計スクリプト。
 *
 * 対象: 東京23区 (特別区) × 2024 Q1〜Q4。多摩26市・町村・島しょ部は対象外。
 * 特別区は独立自治体のため、政令市の親市集約は行わず区コードでそのまま投入する。
 * 処理: 区×四半期ごとに API取得 -> raw バッチ置換 -> quarterly集計 UPSERT。
 *       全区完了後に annual (2024年通年) 集計を UPSERT。
 *       DB接続は区ごとに張り直す (長時間接続の read ETIMEDOUT 回避)。
 *
 * 実行: node --env-file=.env.local scripts/import-reinfolib-tokyo23-2024.mjs
 *
 * 注意:
...
// 東京23区 (特別区) の5桁JISコード。annual集計はこの全23区が対象。
const ALL_JIS = [
...
try {
  ...
  console.log(`東京都 ${cities.length}市 (今回処理分) × ${QUARTERS.length}四半期 を処理`);

  // 市ごとに接続を張り直して raw投入 + quarterly集計 (1市分の処理ごとにDB接続を閉じる)。
  // 一過性のネットワークエラー (EHOSTUNREACH / ETIMEDOUT 等) に備え、市単位で最大3回リトライ。
  // raw はバッチ置換・stats は UPSERT のため、リトライしても冪等。
  const perCity = {};
  for (const c of cities) {
    let lastErr;
    for (let attempt = 1; attempt <= 3; attempt++) {
      const stat = { jis: c.jis, code6: c.code6, q: {}, delete: 0, insert: 0, quarterly: 0 };
      const client = await newClient();
      try {
        for (const q of QUARTERS) {
          const r = await processCityQuarter(client, c.code6, c.name, c.jis, q);
          stat.q[q] = r.apiCount;
          stat.delete += r.deleteCount;
          stat.insert += r.insertCount;
          stat.quarterly += r.quarterlyRows;
          console.log(`[OK] ${c.name} Q${q} api=${r.apiCount} del=${r.deleteCount} ins=${r.insertCount} q-stats=${r.quarterlyRows}`);
        }
        perCity[c.name] = stat;
        apiRequests += QUARTERS.length;
        lastErr = null;
        break;
      } catch (e) {
        lastErr = e;
        console.error(`[RETRY] ${c.name} attempt ${attempt} 失敗: ${e.message}`);
        await new Promise((res) => setTimeout(res, 3000 * attempt));
      } finally {
        try { await client.end(); } catch {}
      }
    }
    if (lastErr) throw new Error(`${c.name} が3回失敗: ${lastErr.message}`);
  }
  ...
  console.log("=== raw: property_type別 (東京23区/2024) ===");
  ...
  console.log("=== raw: price_category別 (東京23区/2024) ===");
  console.table((await client.query(
  ...
  console.log("=== annual: is_low_sample 件数 (東京23区) ===");
  console.table((await client.query(
  ...
  console.log("=== 表示対象: annual transaction で表示3種を持つ東京23区の数 ===");
